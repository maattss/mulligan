...
function cumulativeMetric(player: CompetitionPlayer) {
  const c = competition.value
  if (!c) return 0
    ...
    <section class="flex-1 overflow-y-auto px-4 pt-3 pb-3">
      <template v-if="!isScramble">
        <PlayerScoreRow
          ...
          :cumulative-metric="cumulativeMetric(player)"
          :tee-dot-class="teeDotClass(player.teeSnapshot.color)"
